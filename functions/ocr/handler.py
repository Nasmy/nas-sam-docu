    ...
    with database.get_session() as session:
        try:
            ...
            if document_type == DocumentTypes.PDF.value and file_id and four_digit_page_no:
                ...
                upload_key = f"{file_id}/{file_id}-page-{metadata['page_number']}-ocr.pkl"
                pickled_data = pickle.dumps(predict)
                s3_dd.s3_put_object(bucket=dest_bucket_name, key=upload_key, body=pickled_data, metadata=metadata)
